    ...
    <ScrollView>
      <View style={styles.parent}>
        ...
        {loading ? ( // Display activity indicator if loading is true
          <ActivityIndicator size="large" color={Color.gray200} />
        ) : (
          <FlatList
            horizontal={true}
            showsHorizontalScrollIndicator={false}
            data={searchQuery ? filteredFoodTypes : formattedCategories}
            renderItem={({ item }) => (
              <FoodType
                foodType={item.title.toUpperCase()}
        ...
        <Text style={budget.amount > 0 ? styles.budgetText : styles.noslider}>
          {" "}
          {budget.amount > 0 ? "How much to spend?" : "No budget set"}
        </Text>
        <Text
          style={budget.amount > 0 ? styles.subText : styles.noSliderSubText}
        >{`${budget.amount} left`}</Text>
